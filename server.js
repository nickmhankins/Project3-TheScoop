...
function newComment(url, request) {
...
function deleteComment(url, request) {
  ...
  if (savedComment && id) {
    ...
    const userIndex = database.users[savedComment.username].commentIds;
    userIndex.splice(userIndex.indexOf(id), 1);

    const articleIndex = database.articles[savedComment.articleId].commentIds;
    articleIndex.splice(articleIndex.indexOf(id), 1);
...
function saveDatabase() {
  let save = require('write-yaml');
  save.sync('.database.yml', database)
}
